...
python3.11 -m pip install unicorn pyyaml types-pyyaml numpy iced-x86 protobuf==3.20 toml pydot xxhash; # For revizor; Install on python3.11 !!!
...
python2.7 -m pip install scons==$SCONS_VERSION protobuf==3.18 pydot virtualenv graphviz; # CleanupSpec
...
if [[ -n "$AUTO_RUN" ]]; then
    if [[ "${AUTO_RUN,,}" == "fuzz" ]]; then
      ...
      $BENCHMARK_SH CleanupSpec "$TEST_CASES" "$INPUTS" "$PARALLEL_INSTANCES" &> $RVZR_DIR/bench_sh.out;
    else
