...
class Input(np.ndarray):
    ...
    @staticmethod
    def from_bytes(b: bytes, seed: int):
        from service import LOGGER # Prevent circular import
        '''
        Recreate Input from bytes (obtained via self.tobytes()).

        This will only work properly if CONF.input_register_region_size, etc.
        haven't changed since tobytes() was called.
        '''
        input_ = Input()
        np.copyto(input_, np.frombuffer(b, dtype=np.uint64))
        input_.seed = seed
        LOGGER.dbg_fuzzer(f"from_bytes: {len(input_)} values loaded. 8 bytes per value.")
        return input_
...
class Generator(ABC):
    ...
    def parse_existing_test_case(self, asm_file: str) -> TestCase:
        """
        Generates the input test case from Revizor.
    ...
    # @abstractmethod
    # def parse_existing_input(self, input_pickle_path: str) -> TestCase:
    #     """
    #     Generates the program test case from Revizor.
    #     Read an input from a pickle file and create a complete TestCase object based on it.
    #     Used instead of create_test_case when Revizor works with a user-provided input case.
    #     """
    #     pass
...
class InputGenerator(ABC):
    ...
    @abstractmethod
    def parse_existing_input(inputcase_path: str) -> List[Input]:
        pass
...
class Model(ABC):
    ...
    @abstractmethod
    def trace_test_case(self, inputs: List[Input], nesting: int, round: int) -> List[CTrace]:
        pass
