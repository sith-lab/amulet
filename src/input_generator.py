...
class RandomInputGenerator(InputGenerator):
    ...
    def generate(self, seed: int, count: int) -> List[Input]:
        if seed == 0:
            ...
            # LOGGER.info("input_gen", str(seed)) # debug

        generated_inputs = []
        for i in range(count):
            input_, seed = self._generate_one(seed)
            # LOGGER.dbg_fuzzer(f"generate: {len(input_)} values generated for input. 8 bytes per value")
    ...
    def parse_existing_input(self, inputcase_path: str) -> List[Input]:
        assert os.path.isfile(inputcase_path), f"Input pickle at {inputcase_path} does not exist!"
        with open(inputcase_path, "rb") as file_:
            obj = pickle.load(file_)
            assert isinstance(obj, list), "bad datatype in pickle file"
            return [Input.from_bytes(input, i) for i, input in enumerate(obj)]
