...
class Postprocessor:
    ...
    def _get_all_violations(self, fuzzer: Fuzzer, test_case: TestCase,
        ...
        htraces: List[HTrace] = fuzzer.executor.trace_test_case(inputs, 0) # Don't restore from checkpoint
    ...
    def minimize(self, test_case_asm: str, inputcase: str, outfile: str, num_inputs: int, add_fences: bool):
        # initialize fuzzer
        ...
        if CONF.gem5_orchestration == 'ipc' and inputcase:
            pickle_files = inputcase.split(',')
            if len(pickle_files) != 2:
                print('Error: Expected inputcase to be of the form <reference pickle>,<primer pickle>')
                return
            gen = RandomInputGenerator()
            boosted_inputs = gen.parse_existing_input(pickle_files[0]) + gen.parse_existing_input(pickle_files[1])
        else:
            inputs: List[Input]
            if inputcase:
                # OK to boost inputs after in this case; We are trying to mess with the test case!
                inputs = fuzzer.input_gen.parse_existing_input(inputcase)
            else:
                inputs = fuzzer.input_gen.generate(CONF.input_gen_seed, num_inputs)
