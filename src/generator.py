...
class ConfigurableGenerator(Generator, abc.ABC):
    ...
    def create_test_case(self, asm_file: str, i) -> TestCase:
        # print(random.getstate(), file=open('{}/{}/test_case_{}_state.txt'.format(CONF.debug_dir, CONF.test_case, i), 'a+'))
...
class RandomGenerator(ConfigurableGenerator, abc.ABC):
    ...
    def generate_mem_operand(self, spec: OperandSpec, _: Instruction) -> Operand:
        if spec.values:
            address_reg = random.choice(sorted(spec.values))
        else:
            address_reg = random.choice(sorted(self.register_set.registers[64]))
    ...
    def generate_agen_operand(self, spec: OperandSpec, __: Instruction) -> Operand:
        ...
        reg1 = random.choice(sorted(self.register_set.registers[64]))
        if n_operands == 1:
            return AgenOperand(reg1, spec.width)

        reg2 = random.choice(sorted(self.register_set.registers[64]))
        if n_operands == 2:
...
class X86Generator(ConfigurableGenerator, abc.ABC):
    ...
    # Compile for model (NOT executor)
    ...
    def _parse_instruction(self, line: str, li: int, instruction_map: Dict) -> Instruction:
        ...
        # extract operands
        operands_raw = remove_prefix(line, name).split(",")
    ...
    def map_addresses(self, test_case: TestCase, bin_file: str) -> None:
        ...
        test_case.last_address = address_list[-1]
...
class X86SandboxPass(Pass):
    ...
    def __init__(self):
        ...
        LOGGER.dbg_model("generator::X86SandboxPass", f"Using memory of size {input_memory_size}")
