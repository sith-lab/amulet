...
from service import STAT, LOGGER, bit_count
import json
...
class EquivalenceAnalyser(Analyser):
    ...
    def filter_violations(self,
        ...
        for eq in equivalence_classes:
            table[eq.ctrace] = []
            for m in eq.measurements:
                table[eq.ctrace].append({'input': str(m.input_),'input_id': m.input_id, 'htrace': m.htrace})
            x = table[eq.ctrace]
            x = sorted(x, key=lambda x: x['htrace'])
            table[eq.ctrace] = x

        json.dump(table, open('{}/{}/table.json'.format(CONF.debug_dir, CONF.test_case), 'w'))
    ...
    def check_if_all_subsets(htraces: List[HTrace]) -> bool:
        # Not sure if negative htraces ever actually happen, but there was some
        # code to deal with them oddly.
        max_htrace = max(htraces, key = abs)
        for htrace in htraces:
            htrace = abs(htrace)
            if (htrace & max_htrace) != htrace:
                return False
